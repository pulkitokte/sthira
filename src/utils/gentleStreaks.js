...
// ── Date extraction per source (read-only) ───────────────────────────────────
// Each function returns an array of YYYY-MM-DD date strings from existing data.

function getGratitudeDates() {
  const data = safeRead("sthira_gratitude_garden");
  if (!Array.isArray(data)) return [];
  return data.map((e) => e.date).filter(Boolean);
}
...
export const CONSISTENCY_STATE = {
  FIRST_TIME:    "first-time",     // No reflections ever
  REFLECTED_TODAY: "today",        // Reflected today
  RETURNING:     "returning",      // 7+ days since last reflection
  ONGOING:       "ongoing",        // Has history, reflected recently, not today
};

/**
 * Classify the user's current consistency state.
 * Used to select the appropriate special-state message.
 */
...
/**
 * Get the appropriate headline message for the current state.
 * Falls back to the rotating daily gentle message for the "ongoing" state.
 */
...
/**
 * Build the complete gentle consistency summary for display.
 * Single entry point for the Home card.
 */
export function buildGentleConsistencySummary() {
  ...
  return {
    ...
    state,
    headline,
    // Secondary message shown alongside the headline, except when they would duplicate
    secondaryMessage:
